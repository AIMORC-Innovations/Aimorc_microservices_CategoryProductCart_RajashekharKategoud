...
public interface ProductRepository extends JpaRepository<Products,Integer>
{
	...
	public Products findDetailsByProductId(@Param("product_id") int product_id);
		 ...
		 public List<Map<String, Object>> findorders(@Param ("product_id") Optional<Orders> product_id);
	    	...
	    	@Query(value="select p.product_id, p.product_name ,p.max_quantity, p.product_description , p.product_price, c.category_name, c.category_id from  product p join category c on c.category_id = p.category_id where p.category_id = ?", nativeQuery = true)
			public List<Products> findProducts(@Param("category_id") int category_id);
	    	
	    	@Modifying
			@Query(value="select p.product_id, p.product_name ,p.max_quantity,p.status, p.product_description , p.product_price, p.category_id from product p where p.istrendy=true",nativeQuery = true) 
			  public List<Products> getNewproducts();
	    	@Modifying
			@Query(value="select p.product_id, p.product_name ,p.max_quantity,p.status, p.product_description , p.product_price, p.category_id from product p where p.isnextrelease=true",nativeQuery = true) 
			  public List<Products> getNewRelease();
	    	  
	    	
			@Query(value="select p.product_id, p.product_name ,p.max_quantity, p.product_description , p.product_price, p.category_id from product p where p.category_id=:category_id ",nativeQuery = true)//Limit 2
	    	public Page<Products> findProductByPage(@Param("category_id")int category_id, Pageable Pageable);
	    	 
			@Query(value="select product_id, product_name , product_description ,max_quantity, status, product_price, category_id from product where product_id=:product_id",nativeQuery = true) 
			...
			@Query(value="select p.product_id, p.product_name ,p.max_quantity, p.product_description , p.product_price,  p.category_id from  product p   where p.category_id = :category_id", nativeQuery = true)
